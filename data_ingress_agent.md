...
3. Verify the decision token matches `RUN_DAILY`.

If the decision is missing, stale, or not `RUN_DAILY`:
STOP IMMEDIATELY. REFUSE EXECUTION.

Only `RUN_DAILY` authorizes the standard ingestion pipeline.
For recovery operations, refer to `recovery_agent.md`.
...
After each successful daily pipeline run, DATA_INGRESS writes:

`ANTI_GRAVITY_DATA_ROOT/freshness_index.json`

This is built by `engines/ops/build_freshness_index.py` and records the last
valid timestamp per (symbol, broker, timeframe) combination across all 234+
RESEARCH datasets. Consumers (e.g. Trade_Scan) read this single file to detect
stale data without scanning all MASTER_DATA folders.

The freshness index is a generated artifact — do NOT manually edit it.
It is rebuilt automatically at the end of `engines/ops/daily_pipeline.py`.
